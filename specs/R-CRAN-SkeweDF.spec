%global __brp_check_rpaths %{nil}
%global packname  SkeweDF
%global packver   0.1.0
%global rlibdir   /usr/local/lib/R/library

Name:             R-CRAN-%{packname}
Version:          0.1.0
Release:          1%{?dist}%{?buildtag}
Summary:          Optimization of Skewed Distributions with Birth-Death Processes

License:          GPL-3
URL:              https://cran.r-project.org/package=%{packname}
Source0:          %{url}&version=%{packver}#/%{packname}_%{packver}.tar.gz


BuildRequires:    R-devel
Requires:         R-core
BuildRequires:    R-parallel >= 3.6.3
BuildRequires:    R-methods >= 3.6.3
BuildRequires:    R-grDevices >= 3.6.3
BuildRequires:    R-graphics >= 3.6.3
BuildRequires:    R-utils >= 3.6.3
BuildRequires:    R-CRAN-optimr >= 2019.12.4
BuildRequires:    R-CRAN-stringr >= 1.4.0
BuildRequires:    R-CRAN-Rcpp >= 1.0.5
BuildRequires:    R-CRAN-dplyr >= 1.0.2
BuildRequires:    R-CRAN-zipfR >= 0.6.66
BuildRequires:    R-CRAN-matrixStats >= 0.56.0
BuildRequires:    R-CRAN-purrr >= 0.3.4
Requires:         R-parallel >= 3.6.3
Requires:         R-methods >= 3.6.3
Requires:         R-grDevices >= 3.6.3
Requires:         R-graphics >= 3.6.3
Requires:         R-utils >= 3.6.3
Requires:         R-CRAN-optimr >= 2019.12.4
Requires:         R-CRAN-stringr >= 1.4.0
Requires:         R-CRAN-Rcpp >= 1.0.5
Requires:         R-CRAN-dplyr >= 1.0.2
Requires:         R-CRAN-zipfR >= 0.6.66
Requires:         R-CRAN-matrixStats >= 0.56.0
Requires:         R-CRAN-purrr >= 0.3.4

%description
Implementations of models which follow the Kolmogorov Birth-Death process
framework and functions which utilize these Kolmogorov Birth-Death process
models for analysis of skewed distribution functions.

%prep
%setup -q -c -n %{packname}

# fix end of executable files
find -type f -executable -exec grep -Iq . {} \; -exec sed -i -e '$a\' {} \;
# prevent binary stripping
[ -d %{packname}/src ] && find %{packname}/src -type f -exec \
  sed -i 's@/usr/bin/strip@/usr/bin/true@g' {} \; || true
# don't allow local prefix in executable scripts
find -type f -executable -exec sed -Ei 's@#!( )*/usr/local/bin@#!/usr/bin@g' {} \;

%build

%install

mkdir -p %{buildroot}%{rlibdir}
%{_bindir}/R CMD INSTALL -l %{buildroot}%{rlibdir} %{packname}
test -d %{packname}/src && (cd %{packname}/src; rm -f *.o *.so)
rm -f %{buildroot}%{rlibdir}/R.css
# remove buildroot from installed files
find %{buildroot}%{rlibdir} -type f -exec sed -i "s@%{buildroot}@@g" {} \;

%files
%{rlibdir}/%{packname}
