...
the Xing et al paper. In a nutshell, a cropland connectivity analysis
combines information from maps of cropland density, estimates the relative
likelihood of pathogen movement between cropland locations in the area of
interest, and applies network analysis to calculate the connectivity of
cropland locations. The functions of geohabnet are built to conduct a
cropland connectivity analysis relying on geographic parameters (spatial
...
to cropland connectivity analysis are a) Capability to easily calculate
the connectivity of locations in a landscape using a single function, such
as sensitivity_analysis() or msean(). b) As backbone datasets, the
geohabnet package supports the use of two publicly available global
datasets to calculate cropland density. The backbone datasets in the
geohabnet package include crop distribution maps from Monfreda, C., N.
...
Users can also provide any other geographic dataset that represents host
density. c) Because the geohabnet package allows R users to provide maps
of cropland density (as originally in Xing et al (2021)), host landscape
density (representing the geographic distribution of either crops or wild
